...
var (
	userAgent = fmt.Sprintf("go-mongodbopsmanager/%s (%s;%s)", opsmngr.ClientVersion, runtime.GOOS, runtime.GOARCH)
)
...
func (c *Config) Do(ctx context.Context, req *http.Request, v interface{}) (*opsmngr.Response, error) {
	resp, err := opsmngr.DoRequestWithClient(ctx, c.client, req)
	if err != nil {
	...
	r := &opsmngr.Response{Response: resp}
	body := resp.Body
