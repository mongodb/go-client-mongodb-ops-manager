...
func TestVersionManifest_Get(t *testing.T) {
	...
	defer teardown()
	const version = "test.json"

	mux.HandleFunc(fmt.Sprintf("https://opsmanager.mongodb.com/static/version_manifest/%s", version), func(w http.ResponseWriter, r *http.Request) {
	...
	manifest, _, err := client.VersionManifest.Get(ctx, version)
	if err != nil {
	...
	if diff := deep.Equal(manifest, expected); diff != nil {
		t.Error(diff)
	}
}
